...
geocode_address_pharmacy <- function(address, api_key) {
...
process_data <- function(data, api_key) {
  ...
  for(i in 1:nrow(data)) {
    ...
    result <- geocode_address_pharmacy(
...
write_csv(result_df, "geocoded_addresses_p_temp.csv")
...
write_csv(result_df, "geocoded_addresses_p_final.csv")